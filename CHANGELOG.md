...
### Fixed
- **Listener teardown no longer hangs on Linux**: `DistributedRegistry.stopSync()` now calls `shutdown()` before `close()` on the listening socket. Closing alone never wakes a thread already blocked in `accept()` on Linux (BSD/macOS do wake it, which is why the hang went unnoticed), so any embedder stopping sync deadlocked on the listener join. A 1 s receive-timeout watchdog on the listening socket backstops any missed wakeup.
- **Windows targets compile again**: `posix.SOCK.CLOEXEC` and `posix.setsockopt` do not exist on Windows. The socket shim now routes through a dedicated Winsock backend (`extern "ws2_32"` with one-time `WSAStartup`) and exposes `sock_cloexec`, `shutdown`, `setTimeoutMs`, and `setReuseAddr` helpers used by the distributed registry.
- **Tests link libc explicitly**: the vigil module sets `link_libc`, so `zig build test` compiles on Linux and Windows hosts instead of only inside parent projects that link libc themselves.
