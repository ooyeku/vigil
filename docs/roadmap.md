...
- v2.3.0 has shipped: ring-buffer mailboxes with lazy expiry and condition
  ...
  baselines; see `benchmarks/vigil_bench/README.md`.
- v3.0.0 has shipped: one API convention (snapshot/metrics), injected
  telemetry with no global state, graceful drain, complete runtime
  snapshots, policy telemetry, Erlang-correct restart semantics, the soak
  and chaos stability battery, the task-oriented documentation suite, all
  nine production examples, and the 2.x migration guide. The roadmap is
  complete: Vigil is a high-performance, observable, fault-tolerant
  production runtime for resilient Zig services.
